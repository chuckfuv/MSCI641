...
def train_vali_split(dataset, training = 0.8, base_dir="splits"):
...
def get_stances_for_folds(dataset,training_ids,hold_out):
    stances_training = []
    stances_hold_out = []
        ...
        if stance['Body ID'] in hold_out:
            stances_hold_out.append(stance)
                ...
                if stance['Body ID'] in training_ids:
                    stances_training.append(stance)

    return stances_training,stances_hold_out
